...
class FileDiff:
    ...
    deleted_lines: list[tuple[int, str]] = field(
        default_factory=list
    )  # (line_num, content)
    ...
    for patched_file in patch_set:
        ...
        for hunk in patched_file:
            for line in hunk:
                ...
                if (
                    line.is_added or line.is_context
...
def find_nearest_valid_line(
    mapping: DiffLineMapping, target_line: int, max_distance: int = 5
) -> int | None:
    """
...
def validate_finding_lines(
    findings: list[dict], mappings: dict[str, DiffLineMapping], max_distance: int = 5
...
def should_review_file(filename: str) -> bool:
    ...
    # Check extension
    return all(not filename.lower().endswith(ext) for ext in SKIP_EXTENSIONS)


def filter_files(
    files: list[FileDiff], include_deletions: bool = False
) -> list[FileDiff]:
