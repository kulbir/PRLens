...
def format_findings_markdown(state: ReviewState) -> str:
    ...
    if state.security_findings:
        ...
        for f in state.security_findings:
            ...
            lines.append(f"| **{f.severity}** | {f.line or '-'} | {desc} | {fix} |")
    ...
    if state.general_findings:
        ...
        for f in state.general_findings:
            desc = (f.description[:60] + "...") if len(f.description) > 60 else f.description
            lines.append(f"| {f.severity} | {f.line or '-'} | {f.category} | {desc} |")
...
def post_review_node(state: ReviewState) -> dict:
    ...
    This node reads: repo, pr_number, findings, summary, *_findings
    This node updates: review_posted, review_id, error
    """
    logger.info("📝 Posting review to GitHub...")
    
    try:
        # Build detailed markdown report with all findings
