...
def merge_findings(state: ReviewState) -> dict:
    ...
    if unique_findings:
        summary = (
            f"Found {len(unique_findings)} issue(s): "
            + ", ".join(summary_parts)
        )
    else:
...
def format_findings_markdown(state: ReviewState) -> str:
    ...
    if state.security_findings:
        ...
        for f in state.security_findings:
            ...
            fix = (
                (f.fix[:40] + "...")
                if f.fix and len(f.fix) > 40
                else (f.fix or "-")
            )
            path = f.path or "-"
            lines.append(
                f"| **{f.severity}** | {path} | {f.line or '-'} "
                f"| {desc} | {fix} |"
            )
    ...
    lines.append("\n---")
    lines.append(
        "*Generated by [PRLens](https://github.com/kulbir/PRLens) 🤖*"
    )
...
def should_post_review(state: ReviewState) -> str:
    ...
    findings = (
        state.get("findings", []) if isinstance(state, dict) else state.findings
    )

    if findings:
        logger.info(
            "🔀 Decision: %d issues found → posting review", len(findings)
        )
        return "post_review"
